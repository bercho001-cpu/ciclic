...
El siguiente script DDL crea todas las tablas, relaciones, índices, enums y políticas de seguridad **Row Level Security (RLS)** para aislar estrictamente los datos de cada usuario autenticado y permitir el acceso de lectura anónima a cotizaciones públicas compartidas mediante token.
...
export const InstallmentScheduleSchema = z.object({
  installments: z.array(InstallmentItemSchema).min(1).refine((items) => {
